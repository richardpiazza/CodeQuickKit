...
public class Log {
    ...
    public enum Level: String, Codable, CaseIterable {
        ...
        /// A rankable value used in determining minimums.
        public var rank: Int {
            switch self {
            case .debug: return 0
            case .info: return 1
    ...
    public static let `default`: Log = Log(name: "log.txt")
    ...
    /// A collection of interested parties
    private var observers: [LogObserver] = []
    
    public init() {
        
    }
    
    public convenience init(name: String, maxBytes: UInt? = Log.oneMB) {
        self.init()
        self.fileUrl = Log.fileDirectory?.appendingPathComponent(name)
        self.maxBytes = maxBytes
    }
    
    public convenience init(fileUrl: URL, maxBytes: UInt? = Log.oneMB) {
        self.init()
    ...
    public func addObserver(_ observer: LogObserver) {
        guard observers.contains(where: { $0.observerId == observer.observerId }) == false else {
            return
        }
    ...
    public func removeObserver(_ observer: LogObserver) {
        guard let index = observers.firstIndex(where: { $0.observerId == observer.observerId }) else {
            return
        }
